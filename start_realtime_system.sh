...
# 실시간 촬영 스크립트 선택
echo "🎯 실시간 오버레이 촬영 스크립트를 선택하세요:"
echo "1) record_realtime.py (기본 실시간 오버레이)"
echo "2) record_realtime_v2.py (안정적인 실시간 오버레이 - 권장)"
echo ""
read -p "선택 (1 또는 2): " choice

case $choice in
    1)
        RECORD_SCRIPT="record_realtime.py"
        echo "✅ 기본 실시간 오버레이 선택됨"
        ;;
    2)
        RECORD_SCRIPT="record_realtime_v2.py"
        echo "✅ 안정적인 실시간 오버레이 선택됨"
        ;;
    *)
        echo "❌ 잘못된 선택입니다. 기본값을 사용합니다."
        RECORD_SCRIPT="record_realtime_v2.py"
        ;;
esac
